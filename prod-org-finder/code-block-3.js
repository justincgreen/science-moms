...
const AZ = {
	...
	ideology: {
		...
		liberalGroupLink: 'https://www.climaterealityproject.org/chapters-program?_ga=2.101665862.632263872.1643597658-1114978757.1630074298#arizona',
	...
	faith: {
		yesFaithGroupTitle: 'Interfaith Power & Light Arizona',
...
const GA = {
	...
	ideology: {
		...
		conservativeGroupTwoTitle: 'Citizens for Responsible Energy Solutions',
		conservativeGroupTwoDescription: 'Citizens for Responsible Energy Solutions engages Republican policymakers and the public about responsible, conservative solutions to address our nation\'s energy, economic and environmental security while increasing America\'s competitive edge.',
		conservativeGroupTwoLink: 'https://cresenergy.com/',
		moderateGroupTitle: 'Environmental Defense Fund',
		moderateGroupDescription: 'The Environmental Defense Fund creates transformational solutions to climate change and other environmental problems.',
		moderateGroupLink: 'https://www.edf.org/climate'
	},
	...
	faith: {
		yesFaithGroupTitle: 'Interfaith Power and Light Georgia',
...
const NC = {
	...
	faith: {
		yesFaithGroupTitle: 'Interfaith Power & Light North Carolina',
...
const PA = {
	...
	ethnicity: {
		hispanicGroupTitle: 'Green Latinos Pennsylvania',
		...
		hispanicGroupLink: 'https://www.greenlatinos.org/climate-and-clean-air',
		africanAmericanGroupTitle: 'NAACP Environmental Climate Justice',
		africanAmericanGroupDescription: 'NAACP\'s Environmental and Justice program works to reduce harmful emissions, particularly greenhouse gases, advances energy efficiency and clean energy and strengthen community resilience and livability.',
		africanAmericanGroupLink: 'https://naacp.org/know-issues/environmental-climate-justice'
	},
	faith: {
...
const WI = {
	...
	ethnicity: {
		hispanicGroupTitle: 'Green Latinos Wisconsin',
		hispanicGroupDescription: 'Green Latinos is a coalition of Latino leaders committed to addressing national, regional and local environmental, natural resources and conservations issues that affect the health and welfare of the U.S. Latino community.',
		hispanicGroupLink: 'https://www.greenlatinos.org/climate-and-clean-air',
		africanAmericanGroupTitle: 'NAACP Wisconsin',
		africanAmericanGroupDescription: 'NAACP\'s Environmental and Justice program works to reduce harmful emissions, particularly greenhouse gases, advances energy efficiency and clean energy and strengthen\'s community resilience and livability.',
		africanAmericanGroupLink: 'https://naacp.org/know-issues/environmental-climate-justice'
	},
	faith: {
...
const general = {
	...
	faith: {
		faithGroupOneTitle: 'Interfaith Power and Light',
		faithGroupOneDescription: 'Interfaith Power and Light works with hundreds of congregations of all faiths to save energy, go green and respond to climate change.',
		faithGroupOneLink: 'https://www.interfaithpowerandlight.org/',
		faithGroupTwoTitle: 'Catholic Climate Covenant',
		faithGroupTwoDescription: 'Catholic Climate Covenant educates and equips Catholics to take action on climate change',
		faithGroupTwoLink: 'https://catholicclimatecovenant.org/about/work',
		faithGroupThreeTitle: 'Evangelical Environmental Network',
		faithGroupThreeDescription: 'The Evangelical Environmental Network is a ministry that educates, inspires, and mobilizes Christians in their effort to care for God\'s creation.',
		faithGroupThreeLink: 'https://creationcare.org'
	}
}

// Result object gets populated based on selected quiz responses
let result = { 
	state: '', 
	ideology: '',
	school: '',
...
const paginationUI = (e) => {
	...
	if(e.target.classList.contains('question-count-2')) {
		if(e.target.classList.contains('active')) {
			...
			// Add active class to current question count
			e.target.classList.add('active');
			quizSection.style.backgroundColor = '#f9beba';
	...
	if(e.target.classList.contains('question-count-4')) {
		if(e.target.classList.contains('active')) {
			...
			// Add active class to current question count
			e.target.classList.add('active');
			quizSection.style.backgroundColor = '#dde3fd';
	...
	if(e.target.classList.contains('question-count-5')) {
		if(e.target.classList.contains('active')) {
			...
			// Add active class to current question count
			e.target.classList.add('active');
			quizSection.style.backgroundColor = '#f9dbd9';
...
// pagination count currently disabled - uncomment code below to enable if needed
...
// School checkbox behavior
const yesSchoolCheckboxWrapper = document.querySelector('.checkbox-wrapper-yes-school');
const noSchoolCheckboxWrapper = document.querySelector('.checkbox-wrapper-no-school');
const yesSchoolLabel = document.querySelector('.yes-school-label');
const noSchoolLabel = document.querySelector('.no-school-label');
const yesSchoolCheckbox = document.querySelector('.yes-school');
const noSchoolCheckbox = document.querySelector('.no-school');

yesSchoolCheckboxWrapper.addEventListener('click', () => {
	yesSchoolCheckbox.checked = true;
	noSchoolCheckbox.checked = false;
	noSchoolLabel.classList.remove('selected');
	yesSchoolLabel.classList.add('selected');
});

yesSchoolCheckbox.addEventListener('click', () => {
	noSchoolCheckbox.checked = false;
	noSchoolLabel.classList.remove('selected');
	yesSchoolLabel.classList.add('selected');
});

noSchoolCheckboxWrapper.addEventListener('click', () => {
	noSchoolCheckbox.checked = true;
	yesSchoolCheckbox.checked = false;
	yesSchoolLabel.classList.remove('selected');
	noSchoolLabel.classList.add('selected');
});

noSchoolCheckbox.addEventListener('click', () => {
	yesSchoolCheckbox.checked = false;
	yesSchoolLabel.classList.remove('selected');
	noSchoolLabel.classList.add('selected');
});

// Faith checkbox behavior
const yesFaithCheckboxWrapper = document.querySelector('.checkbox-wrapper-yes-faith');
const noFaithCheckboxWrapper = document.querySelector('.checkbox-wrapper-no-faith');
const yesFaithLabel = document.querySelector('.yes-faith-label');
const noFaithLabel = document.querySelector('.no-faith-label');
const yesFaithCheckbox = document.querySelector('.yes-faith');
const noFaithCheckbox = document.querySelector('.no-faith');

yesFaithCheckboxWrapper.addEventListener('click', () => {
	yesFaithCheckbox.checked = true;
	noFaithCheckbox.checked = false;
	noFaithLabel.classList.remove('selected');
	yesFaithLabel.classList.add('selected');
});

noFaithCheckboxWrapper.addEventListener('click', () => {
	noFaithCheckbox.checked = true;
	yesFaithCheckbox.checked = false;
	yesFaithLabel.classList.remove('selected');
	noFaithLabel.classList.add('selected');
});

// Next button (btn-next-five) for cta-section 
const btnNextFive = document.querySelector('.btn-next-five');
const ctaCaptureSection = document.querySelector('.cta-capture-section');
const paginatedCount = document.querySelector('.paginated-count');
// yesFaithCheckbox and noFaithCheckbox constants being pulled from above (faith checkbox behavior)
//  questionAll and quizSection constant being pulled from higher above ( Next button behavior)

// Email Collection form
const emailForm = '<?php do_shortcode("[wpforms id="5149" title="false"]");?>';
...
btnNextFive.addEventListener('click', () => {
	...
	ctaCaptureSection.classList.remove('hidden');
	paginatedCount.classList.add('hidden');
	quizSection.style.backgroundColor = '#f9beba';
	
	// Set Faith value
	if(yesFaithCheckbox.checked === true) {
		noFaithCheckbox.checked === false;
		result.faith = 'yes';
		console.log(result);
	}else if(noFaithCheckbox.checked === true) {
		yesFaithCheckbox.checked === false;
		result.faith = 'no';
		console.log(result);
	}
...
// Discover button moved to WPForm confirmation message
...
const inputFirstName = document.querySelector('#wpforms-5149-field_1');
const inputLastName = document.querySelector('#wpforms-5149-field_1-last');
const inputEmail = document.querySelector('#wpforms-5149-field_2');

document.addEventListener('click', function(e){
	if(e.target && e.target.id == 'wpforms-submit-5149'){
		// extremely basic form validation
		// just making sure each input has
		// some amount of entered content
		if(inputFirstName.value.length > 0 && inputLastName.value.length > 0 && inputEmail.value.length > 0) {
